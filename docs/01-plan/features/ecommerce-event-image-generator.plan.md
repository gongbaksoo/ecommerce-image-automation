...
│   │   ├── HeroSection.tsx       # 상단 히어로 영역 (배경+문구+연출이미지)
...
[Step 2: 콘텐츠 입력]
  ...
  └── 6) (선택) 레퍼런스 이미지 업로드
        │
        ▼
[Step 3: AI 배경 생성] ← Gemini API (배경 이미지만)
        │
        ▼
[Step 4: HTML/CSS 합성 & 실시간 미리보기]
  ├── Layer 1: AI 배경 이미지 (or 사용자 업로드 배경)
  ├── Layer 2: 메인 상품 연출 이미지
  ├── Layer 3: 문구/가격 HTML 텍스트 (CSS 타이포그래피)
  └── Layer 4: 개별 상품 카드 그리드
        │
        ▼
